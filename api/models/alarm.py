...
async def check_alarmevent(credentials, alarm, max_count):
    async with DatabaseConnection() as conn:
        async with conn.transaction(isolation='repeatable_read'):
            ...
            for recno, row_alarm in enumerate(db_alarm):
                if max_count is not None and recno >= max_count:
                    break
                ...
                for row_cond in db_conds:
                    ...
                    for r_series_prev, r_series in prev_iterator(db_series, include_first=False):
                        if row_cond["log_row_category"] == AlarmCondLogRowCategory.condition:
                            if r_series["value_text"] is not None and r_series["value_text"] == row_cond["value_text"]:
                            ...
                            # todo 1: **********
                            if r_series["value_num"] is not None and r_series["value_num"] > 32025:
