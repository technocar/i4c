...
create table "stat_timeseries" (
    id SERIAL PRIMARY KEY,
    stat integer not null constraint fk_pv references "stat",
    
    after timestamp with time zone NULL,
    before timestamp with time zone NULL,
    duration character varying (200) null,
...
create table "stat_timeseries_filter" (
    id SERIAL PRIMARY KEY,
    timeseries integer not null constraint fk_pv references "stat_timeseries",
